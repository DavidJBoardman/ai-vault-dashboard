...
export function useStep4Geometry2DController() {
  ...
  const acknowledgeStale = (section: "matching" | "reconstruct") => {
    if (section === "matching") {
      const sig = composeSignature([savedRoiSignature, currentPointsSignature]);
      setMatchingUpstreamSig(sig);
      persistMatchingPatch({ upstreamSig: sig });
    } else {
      const sig = composeSignature([savedRoiSignature, currentPointsSignature, currentMatchingResultSignature]);
  ...
  const handleRunReconstruction = useCallback(async () => {
    ...
    try {
      ...
      // cut-typology with an identical result does not flag 4D as stale.
