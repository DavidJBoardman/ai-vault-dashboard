...
// Content fingerprint of the cut-typology RESULT that the bay plan (4D) consumes:
// the best variant plus each boss's resolved cut assignment. Deterministic, so
// re-running 4C with an identical result yields the same signature and does not
// flag 4D as stale.
function matchingResultSignature(
  bestVariantLabel: string | undefined,
  perBoss: Array<{ id: number; matchedAny: boolean; axisCutMatch?: { xCut?: string | null; yCut?: string | null } | null }>
): string {
  const perBossSig = [...perBoss]
    .sort((a, b) => a.id - b.id)
    .map((b) => `${b.id}:${b.matchedAny ? 1 : 0}:${b.axisCutMatch?.xCut ?? ""}/${b.axisCutMatch?.yCut ?? ""}`)
    .join(",");
  return composeSignature([bestVariantLabel, perBossSig]);
}
...
export function useStep4Geometry2DController() {
  ...
  const persistReconstructPatch = useCallback(
    (patch: Partial<NonNullable<Step4Geometry2DState["reconstruct"]>>) => {
      const geometry2d = getLatestStep4Geometry2D().geometry2d;
      const existingReconstruct = geometry2d.reconstruct || {};
      updateStep4Geometry2D({
        reconstruct: {
          ...existingReconstruct,
          ...patch,
  ...
  const currentMatchingResultSignature = useMemo(
    () => matchingResultSignature(templateBestVariantLabel, templatePerBoss),
  ...
  //
  // The points component uses the SAVED points signature (not the live edited
  // points): 4C/4D run on the persisted nodes, so unsaved in-progress edits in
  // 4B must not pre-flag downstream — the flag appears once the points are saved.
  //
  ...
  // corner points are regenerated by the 4A Analyse step. ROI changes are caught
  // directly by 4C/4D below, whose fingerprints include the ROI signature.
  const matchingStale =
    !!templateLastRunAt &&
    (matchingUpstreamSig === undefined ||
      matchingUpstreamSig !== composeSignature([savedRoiSignature, templateSavedPointsSignature]));

  const reconstructStale =
    !!reconstructLastRunAt &&
    (reconstructUpstreamSig === undefined ||
      reconstructUpstreamSig !== composeSignature([savedRoiSignature, templateSavedPointsSignature, currentMatchingResultSignature]));
  ...
  // an upstream input changes again.
  const acknowledgeStale = (section: "matching" | "reconstruct") => {
    if (section === "matching") {
      const sig = composeSignature([savedRoiSignature, templateSavedPointsSignature]);
      setMatchingUpstreamSig(sig);
      persistMatchingPatch({ upstreamSig: sig });
    } else {
      const sig = composeSignature([savedRoiSignature, templateSavedPointsSignature, currentMatchingResultSignature]);
      setReconstructUpstreamSig(sig);
      persistReconstructPatch({ upstreamSig: sig });
    }
  };
  ...
  const handleRunReconstruction = useCallback(async () => {
    ...
    try {
      ...
      setReconstructLayers(nextLayers);
      // Third component is 4C's RESULT signature (not its run id), so re-running
      // cut-typology with an identical result does not flag 4D as stale. The
      // points component uses the SAVED points signature to match the staleness
      // derivation basis (the backend reconstructs from the persisted nodes).
      const reconstructSig = composeSignature([
        savedRoiSignature,
        templateSavedPointsSignature,
        currentMatchingResultSignature,
      ]);
  ...
  const handleRunTemplateMatching = async () => {
    ...
    try {
      ...
      // Re-running cut-typology only invalidates the bay plan if the RESULT
      // changed — an identical re-run leaves 4D fresh.
